...
def carpet_bomb_cpp():
    pristine_code = """#include "inference_manager.h"
...
"""
    ...
    found_count = 0
    # Overwrite every single inference_manager.cpp we can find (except in build cache)
    for root, dirs, files in os.walk("."):
        if "build" in root: continue
        
        if "inference_manager.cpp" in files:
            target = os.path.join(root, "inference_manager.cpp")
            with open(target, "w", encoding="utf-8") as f:
                f.write(pristine_code)
            print(f"SUCCESS: OVERWRITTEN -> {target}")
            found_count += 1
            
    if found_count == 0:
        print("ERROR: No inference_manager.cpp found in the source tree.")
